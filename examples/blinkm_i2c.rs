...
fn init<T: Read + Write + std::fmt::Debug>(board: &Arc<Mutex<Board<T>>>) {
...
fn set_rgb<T: Read + Write + std::fmt::Debug>(board: &Arc<Mutex<Board<T>>>, rgb: [u8; 3]) {
    let mut b = board.lock().expect("lock");
    b.i2c_write(0x09, "n".as_bytes()).expect("i2c write");
    b.i2c_write(0x09, &rgb).expect("i2c write");
}

fn read_rgb<T: Read + Write + std::fmt::Debug>(board: &Arc<Mutex<Board<T>>>) -> Vec<u8> {
	let mut b = board.lock().expect("lock");
...
fn main() {
    ...
    let board = Arc::new(Mutex::new(
        firmata_client_rs::Board::new(Box::new(port)).expect("new board"),
    ));
